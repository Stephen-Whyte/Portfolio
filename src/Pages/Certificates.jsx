...
function Template(Props){
    return (
        <div className="rounded-xl bg-card text-card-foreground overflow-hidden shadow-md hover:shadow-lg transition-shadow border-0 text-center p-4">
            {
                Props.Info.Img ? <img className="flex justify-center w-full h-50 object-cover rounded-t-xl" src={Props.Info.Img} alt={Props.Info.Title} />
                : null
            }
            <div className="p-4">
                <h1 className="font-semibold">{Props.Info.Title}</h1>
                <p>{Props.Info.Desc}</p>
                <p className="text-blue-500 font-semibold">{Props.Info.Date}</p>
                {
                    Props.Info.Link ? 
